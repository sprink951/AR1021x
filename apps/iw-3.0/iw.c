...
int print_candidate_flag = 0;
int cur_found_freq_num = 0;
SIGNAL_LIST *channel_signal[TOTAL_CHAN_MAX] = {NULL, }, *channel_candidate = NULL;
/* add for get max signal strength channel from scanned AP--Wed, May 15 2013*/
MAX_RSSI_CHANNEL max_rssi_channel;
int print_max_rssi_flag = 0;
int totalweight_2g[11]={0,};
...
int main(int argc, char **argv)
{
 ...
 detect:
	...
	if (print_max_rssi_flag)
	{
		printf("\n\n");
		printf("\t\t\tMax RSSI Channel Info:\n\n");
		printf("SSID:%s\tchannel:%d\tfreq:%d\tsignal:%.2f dBm\n\n", max_rssi_channel.ssid, ieee80211_frequency_to_channel(max_rssi_channel.freq), max_rssi_channel.freq, max_rssi_channel.signal/100.0);
	}

	if (print_candidate_flag)
	{
		count_total_weight(channel_candidate);
		printf("\n\nAuto Selected Channel for 2.4G:  %d\n\n",
		find_candidate(totalweight_2g, 11));
		delete_signal_node_candidate(&channel_candidate);
	}
