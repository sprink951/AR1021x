...
int p2p_peer_channels_update(struct p2p_data *p2p,
					    ...
					    struct p2p_device *peer)
{
	...
	if (peer != NULL && peer->channels.reg_classes != 0) {
		struct p2p_channels intersection;
					     ...
					     u8 tie_breaker)
{
	...
	p2p_buf_add_capability(buf, p2p->dev_capab, group_capab);
    wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "p2p_build_go_neg_resp p2p->go_intent=%d\n", p2p->go_intent);
			    ...
			    const u8 *data, size_t len, int rx_freq)
{
...
#ifdef CONFIG_WFD
	if (dev == NULL)
		dev = p2p_add_dev_from_go_neg_req(p2p, sa, data, len, &msg);
	else
		p2p_add_dev_info(p2p, sa, dev, &msg, data, len);
#else
	if (dev == NULL)
	...
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		if (dev) {
		    p2p_peer_channels_update(p2p, dev, msg.channel_list,
