...
int wps_build_primary_WiDi_dev_type(int *dev_type, struct wpabuf *msg)
{
	wpa_printf(MSG_DEBUG, "WPS WiDi:  * Primary Device Type");
	
	wpabuf_put_be16(msg, ATTR_PRIMARY_DEV_TYPE);
	wpabuf_put_be16(msg, WPS_DEV_TYPE_LEN);
	wpabuf_put_data(msg, dev_type, WPS_DEV_TYPE_LEN);
	
	return 0;
}

int wps_build_WiDi_dev_name(char *dev_name, struct wpabuf *msg)
{
	size_t len;

	wpa_printf(MSG_DEBUG, "WPS WiDi:  * Device Name");
	wpabuf_put_be16(msg, ATTR_DEV_NAME);
	len = dev_name ? os_strlen(dev_name) : 0;
	...
	wpabuf_put_be16(msg, len);
	wpabuf_put_data(msg, dev_name, len);

	return 0;
}

int wps_build_primary_dev_type_PassDEV_type(struct wps_device_data *dev, struct wpabuf *msg)
{
	u8 pri_dev_type_orig[8]={0x00,0x01,0x00,0x50,0xF2,0x04,0x00,0x01};
	
	wpa_printf(MSG_DEBUG, "WPS:  * Primary Device Type");
	
	wpabuf_put_be16(msg, ATTR_PRIMARY_DEV_TYPE);
	wpabuf_put_be16(msg, WPS_DEV_TYPE_LEN);
	wpabuf_put_data(msg, pri_dev_type_orig, WPS_DEV_TYPE_LEN);
