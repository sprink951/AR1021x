...
int enable_widi_ve = 0;
u8 fill_sink_info[11]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
#ifdef CONFIG_CUSTOM_WFD
int wps_build_wfa_ext_widi(int enable_WiDi, u8 *sink_info)
{
	if(enable_WiDi && (sink_info == NULL))
		return 1;

	if(!enable_WiDi)/* disable, we should clear the sink info */
		memset(fill_sink_info, 0, sizeof(fill_sink_info));
	else /* enable, we should update the sink info */
		memcpy(fill_sink_info, sink_info, sizeof(fill_sink_info));
	
	enable_widi_ve = enable_WiDi;
		      ...
		      const u8 *auth_macs, size_t auth_macs_count)
{
	...
	if (enable_widi_ve == 1)
	{
		...
		wpabuf_put_be24(msg, WPS_WIDI_SINK_VENDOR_OUI); /* must match with Source OUI */
		wpabuf_put_u8(msg, 0x2);          //REMOTE DISPLAY
		wpabuf_put_be16(msg, 0x0003);     //REMOTE DISPLAY Sink Info
		wpabuf_put_be16(msg, 0x000b);     //Length of data that follows
		
		for (i=0; i < sizeof(fill_sink_info); i++)
		{
		    wpabuf_put_u8(msg, fill_sink_info[i]);   //sink info
		}
