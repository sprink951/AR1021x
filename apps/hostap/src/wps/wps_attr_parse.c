...
static int wps_parse_vendor_ext_widi(u32 config_vendor_id, struct wps_parse_attr *attr, const u8 *pos,
                              u16 len, char *buf_VE)
{
	u32 vendor_id = 0;
	int i = 0;
	...
	if(config_vendor_id == vendor_id){			
			if ( pos[5] == WIDI_TYPE_L2SD_QUERY ){
				wpa_printf(MSG_DEBUG, "%s Receive L2SD_QUERY", __func__);          
				ret = os_snprintf(buf_VE, 100-ret, "%02x-", WIDI_TYPE_L2SD_QUERY);
				l2sd_ta_len = 16;
			}else if ( pos[5]== WIDI_TYPE_L2TA_TRIGGER ){
				wpa_printf(MSG_DEBUG, "%s Receive L2TA_TRIGGER", __func__);                   
				ret = os_snprintf(buf_VE, 100-ret, "%02x-", WIDI_TYPE_L2TA_TRIGGER);
				l2sd_ta_len = 20;
			}else{
				return 0;
			}
			buf_VE += ret;
			ret = os_snprintf(buf_VE, 100-ret, "%02x-", l2sd_ta_len);
			buf_VE += ret;

			for (i=0 ; i < l2sd_ta_len ; i++)
			{
				ret = os_snprintf(buf_VE, 100-ret, "%02x", pos[i]);
				buf_VE += ret;
			}

			return 4; /* return value 4 for match case */
	}
	else{
		wpa_printf(MSG_MSGDUMP, "WPS: Skip unknown Vendor Extension "
			"(Vendor ID %u) Currnet Setting %u", vendor_id, config_vendor_id);
	}
	return 0;
}
...
#ifdef CONFIG_CUSTOM_WFD
static int wps_set_attr_widi(u32 vendor_id, struct wps_parse_attr *attr, u16 type,
                             const u8 *pos, u16 len, char *buf_VE)
{
	int wps_vendor_id;
	
	switch (type) {
		case ATTR_VENDOR_EXT:		 	
			wps_vendor_id = wps_parse_vendor_ext_widi(vendor_id, attr, pos, len, buf_VE);
...
#ifdef CONFIG_CUSTOM_WFD
int wps_parse_msg_widi(u32 vendor_id, const struct wpabuf *msg, struct wps_parse_attr *attr, char *buf_VE)
{
        const u8 *pos, *end;
        u16 type, len;
        u16 prev_type = 0;
...
#endif /* WPS_WORKAROUNDS */
                        wps_vendor_id = wps_set_attr_widi(vendor_id, attr, type, pos, len, buf_VE);
                        if (wps_vendor_id < 0)
                        {
                                return -1;
                        }
