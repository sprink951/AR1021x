...
int wps_is_vendor_id_match(u32 vendor_id, const struct wpabuf *msg, char *buf_VE)
{
    struct wps_parse_attr attr = {0};

    if (wps_parse_msg(msg, &attr) < 0)
         return 0;
    else if (wps_parse_msg_widi(vendor_id, msg, &attr, buf_VE) == 4)
         return 4;    /* "4" indicates WiDi device */
    else
         return 1;
}
 ...
 */
#ifdef CONFIG_CUSTOM_WFD
struct wpabuf * wps_build_probe_req_ie(int widi_enable, char* widi_dev_name,
				       u16 pw_id, struct wps_device_data *dev,
				       const u8 *uuid,
...
#endif /* CONFIG_CUSTOM_WFD */	
{
	...
	if (widi_enable == 1)
	{
		if (wps_build_version(ie) ||
		    ...
		    wps_build_WiDi_dev_name(widi_dev_name, ie) || /* Fill in widi device name */
