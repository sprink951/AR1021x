				   ...
				   const struct wps_credential *cred)
{
	...
	wpa_hexdump_ascii(MSG_DEBUG, "WPS: Store SSID", wpa_s->wps_onging_ssid, wpa_s->wps_onging_ssid_len);
	...
	if (os_memcmp(wpa_s->wps_onging_ssid, cred->ssid, cred->ssid_len) == 0) {
		priority = 1;
	}
	wpa_printf(MSG_DEBUG, "WPS: Priority 0x%x", priority);
...
{
#ifdef CONFIG_CUSTOM_WFD
	WIDI_set_state_up(wpa_s, L2SDTA_WPS_STATUS_FAIL);
#endif /* CONFIG_CUSTOM_WFD */
...
#endif /* CONFIG_P2P */
#ifdef CONFIG_CUSTOM_WFD
	WIDI_set_source_info(wpa_s); 
	WIDI_set_state_up(wpa_s, L2SDTA_WPS_STATUS_SUCCESS);
#endif /* CONFIG_CUSTOM_WFD */  
}
			      ...
			      struct wpa_bss *selected, struct wpa_ssid *ssid)
{
	...
	dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
		...
		wpa_hexdump(MSG_DEBUG, "WPS: UUID of the other BSS",
			    uuid, UUID_LEN);
		if (os_memcmp(bss->bssid, selected->bssid, ETH_ALEN) == 0) {
			wpa_msg(wpa_s, MSG_INFO, "WPS: PBC overlap ignored due to same bssid : "
				MACSTR "(%d) and " MACSTR "(%d)",
				MAC2STR(selected->bssid), selected->freq,
				MAC2STR(bss->bssid), bss->freq);			
		}		
		else if (sel_uuid == NULL || uuid == NULL ||
		    os_memcmp(sel_uuid, uuid, UUID_LEN) != 0) {
		    if ((selected->freq > 5000 && bss->freq > 5000) ||
		        (selected->freq < 5000 && bss->freq < 5000))
		    {
  			    ret = 1; /* PBC overlap */
		    ...
		    {
    		    wpa_msg(wpa_s, MSG_INFO, "WPS: PBC overlap ignored: "
	    	    	MACSTR "(%d) and " MACSTR "(%d)",
		        	MAC2STR(selected->bssid), selected->freq,
			        MAC2STR(bss->bssid), bss->freq);
		    }
