 ...
 * Returns: Matching blacklist entry for the BSSID or %NULL if not found
 */
struct wpa_blacklist * wpa_blacklist_get_p2p(struct wpa_supplicant *wpa_s,
					 const u8 *bssid)
{
	struct wpa_blacklist *e;

	e = wpa_s->p2p_blacklist;
	while (e) {
		if (os_memcmp(e->bssid, bssid, ETH_ALEN) == 0){
			wpa_printf(MSG_ERROR, "found BSSID " MACSTR " in blacklist",
				MAC2STR(bssid));
			return e;
		}
		e = e->next;
 ...
 * changes the order in which they are tried.
 */
int wpa_blacklist_add_p2p(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	struct wpa_blacklist *e;
	...
	os_memcpy(e->bssid, bssid, ETH_ALEN);
	e->count = 1;
	e->next = wpa_s->p2p_blacklist;
	wpa_s->p2p_blacklist = e;
	wpa_printf(MSG_DEBUG, "Added BSSID " MACSTR " into blacklist",
		   MAC2STR(bssid));
 ...
 * Returns: 0 on success, -1 on failure
 */
int wpa_blacklist_del_p2p(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	struct wpa_blacklist *e, *prev = NULL;
 ...
 * @wpa_s: Pointer to wpa_supplicant data
 */
void wpa_blacklist_clear_p2p(struct wpa_supplicant *wpa_s)
{
	struct wpa_blacklist *e, *prev;

	e = wpa_s->p2p_blacklist;
	wpa_s->p2p_blacklist = NULL;
	while (e) {
		prev = e;
		e = e->next;
		wpa_printf(MSG_DEBUG, "Removed BSSID " MACSTR " from "
