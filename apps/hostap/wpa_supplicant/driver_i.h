...
static inline int wpa_drv_scan_cancel(struct wpa_supplicant *wpa_s)
{
	if(wpa_s->driver->scan_cancel)
		return wpa_s->driver->scan_cancel(wpa_s->drv_priv);
	return -1;
}
...
static inline int wpa_drv_wlan_swow_cmd(struct wpa_supplicant *wpa_s,
				     char *cmd, char *buf, size_t buf_len)
{
	if (!wpa_s->driver->wlan_swow_cmd)
		return -1;
	return wpa_s->driver->wlan_swow_cmd(wpa_s->drv_priv, cmd, buf, buf_len);
}
#endif//swow

static inline int wpa_drv_wlan_hb_cmd(struct wpa_supplicant *wpa_s,
				     char *cmd, char *buf, size_t buf_len)

{
	if (!wpa_s->driver->wlan_hb_cmd)
		return -1;
	return wpa_s->driver->wlan_hb_cmd(wpa_s->drv_priv, cmd, buf, buf_len);
}
...
static inline int wpa_drv_wlan_debugfs_cmd(struct wpa_supplicant *wpa_s,
				     char *cmd, char *buf, size_t buf_len)
{
	if (!wpa_s->driver->wlan_debugfs_cmd)
		return -1;
	return wpa_s->driver->wlan_debugfs_cmd(wpa_s->drv_priv, cmd, buf, buf_len);
}
